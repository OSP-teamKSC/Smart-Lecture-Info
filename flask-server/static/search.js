...
//검색 버튼 클릭
function Search(){
    if(document.getElementById('isDemo').checked){
    ...
    xhr.onload = () => {
        if (xhr.status == 200) {
            ...
            ReloadTable(TestLoad(xhr.response));
        } else {
            //failed
        }
    }
}
function TestLoad(datas){
    let _result = []
    for (key in datas) {
        let sb = new Subject(datas[key],true)
        _result.push(sb)
    }
    return _result
}
...
function removeAll(selectedElement)
{
    const L = selectedElement.options.length - 1;
    for(let i = L; i >= 0; i--) {
    ...
    if(IdSeme.value!=="")
    {
        ...
        xhr.onload = () => {
            if (xhr.status == 200) {
                //success
                //console.log(xhr.response);
                UnivList = xhr.response;
            } else {
                //failed
            }
        }
    }
    let optun = document.createElement("option");
    optun.value = 'UN';
    optun.innerHTML = "대학";
    IdUniv.appendChild(optun);
    let optge = document.createElement("option");
    optge.value = 'GE';
    optge.innerHTML = "교양";
    IdUniv.appendChild(optge);

}

IdUniv.onchange = function (){
    let collegeList;
    //하위 카테고리 초기화
    removeAll(IdCollege);
    removeAll(IdMajor);
    ...
    //insert options
    if(IdUniv.value!=="")   {
        collegeList = IdUniv.value === 'UN' ? Object.keys(UnivList): Object.keys(GEList);
