...
const Subject = {"": null,"UN": UnivList, "GE": GEList};
...
const SearchSelect = () => {
    ...
    const [selectDepa, setSelectDepa] = useState("");
    const [selectaddO, setselectaddO] = useState("");

    
    function loadUniv() {
        switch(selectSubj) {
            case "UN" : return ListToOption(Object.keys(Subject[selectSubj]));
            case "GE" : return ListToOption(Object.keys(Subject[selectSubj]));
            default : return null;
        }
    }
    ...
    function loadDepa() {
        if(selectDepa !== "")
        {
            let selectMj = Subject[selectSubj][selectDepa];
            if(selectDepa==="첨성인핵심")
            {
                selectMj = Object.keys(Subject[selectSubj][selectDepa]);
            }
            switch(selectSubj) {
    ...
    function loadCSHS() {
        if(selectDepa === "첨성인핵심" && selectaddO !== "")
        {
            const series = Subject[selectSubj][selectDepa][selectaddO];
    ...
    return <div className="search-select">
        ...
        <div className="search-condition">
            <input type="number" placeholder="개설연도"
            id="year" />
            ...
            <select name="Subject" onChange={(e)=>{ setSelectSubj(e.target.value); setSelectDepa(""); setselectaddO("");}}>
                <option value="">선택</option>
                <option value="UN">대학</option>
                <option value="GE">교양</option>
            </select>
            <select name="university" onChange={(e)=>{ setSelectDepa(e.target.value);  setselectaddO("");}}>
                <option value="">선택</option>
                { loadUniv() }
            </select>
            <select name="department" onChange={(e)=>{ setselectaddO(e.target.value)}}>
                <option value="">선택</option>
                { loadDepa() }
            </select>
            <select name="addOn">
