    ...
    with open(folder+file, 'r') as json_file:    
        ...
        for i in json_line:
            ...
            if i['Schedule'] is not None:
                ...
                i['Schedule'] = ts
            sql_duplicate = "update un set SearchDepartment = concat(SearchDepartment, %s) where ClassCode = (%s)"
            ...
            try:
                ...
                print("Inserting [%s, %s] to 대학"%(i["SubjectName"], i["ClassCode"])) #삽입 성공
                ...
                print("%s %s %s already in 대학"%(i["ClassCode"],i["EstablishedUniversity"],i["EstablishedDepartment"])) #이미 삽입한 값
