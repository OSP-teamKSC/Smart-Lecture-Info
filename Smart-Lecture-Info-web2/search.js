...
const IdGubun = document.getElementById("Gubun");
const IdCollage = document.getElementById("Collage");
...
function setCollage(e)          //단과대학 selelct 생성
{
    ...
    //하위 카테고리 초기화
    IdCollage.options.length = 0;
    IdMajor.options.length = 0;
    init(IdCollage);
    init(IdMajor);
    ...
    if(e.value!=="NULL")
    {
        collageList = Object.keys(Gubun[e.value]);
        categoryChange(IdCollage, collageList);
    }
...
{
    const course = Gubun[IdGubun.value][e.value];
    //초기화
    IdMajor.options.length = 0;
    init(IdMajor);
